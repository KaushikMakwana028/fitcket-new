...
class Api extends CI_Controller
{
    ...
    public function add_city()
    {
        ...
        return $this->output
            ->set_status_header($insert_id ? 200 : 500)
            ->set_content_type('application/json')
            ->set_output(json_encode([
                'status' => (bool) $insert_id,
                'code' => $insert_id ? 200 : 500,
                'message' => $insert_id ? 'City added successfully' : 'Failed to add city',
                'data' => $insert_id ? [
                    'id' => $insert_id,
                    'city' => $city,
                    'state' => $state
    ...
    public function save_profile()
    {
        ...
        $latitude = trim($input['latitude'] ?? $input['lat'] ?? '');
        $longitude = trim($input['longitude'] ?? $input['lng'] ?? '');
        $existing = $this->general_model->getOne('provider', ['provider_id' => $provider_id]);

        if ((empty($latitude) || empty($longitude)) && $address) {
            $ch = curl_init();
        ...
        if ((empty($latitude) || empty($longitude)) && $existing) {
    ...
    public function notifications_get()
    {
        ...
        foreach ($rows as $n) {
            ...
            /* ================= SESSION BOOKING ================= */ elseif ($n['type'] === 'session') {
                $session = $this->db
                    ->select('
