...
public function logout()
{
    ...
    // ✅ 1. Add token to blacklist (so it can’t be reused)
    ...
    // ✅ 2. Remove provider’s FCM token (so they stop receiving notifications)
  ...
  private function verify_jwt($token)
{
        ...
        private function generate_jwt($user)
    {
        $payload = [
            'iss' => base_url(),
            'iat' => time(),
            'exp' => time() + (10 * 365 * 24 * 60 * 60),            // 'exp' => time() + 3600, // 1 minute expiry
