...
public function login_send_otp()
{
    ...
    if ($mobile == "8128966157") {
        $otp = "123456";
        $message = "Your OTP is 123456. (Test Mode)";
        $sms_sent = $this->send_otp_via_sms($mobile, $otp, $message);
...
public function pay_booking()
{
    ...
    return $this->output
        ...
        ->set_output(json_encode([
            'status' => true,
            'code' => 200,
            'message' => 'Order created successfully (notification sent for testing).',
            'data' => [
                'razorpay_key' => $this->razorpay_key_id,
                'order_id' => $razorpayOrder['id'],
                'txnid' => $txnid,
...
public function callback_booking()
{
    ...
    if (empty($input['razorpay_order_id']) || empty($input['razorpay_payment_id']) || empty($input['razorpay_signature']) || empty($input['txnid'])) {
        return $this->output
            ->set_status_header(400)
            ->set_output(json_encode([
                'status' => false,
    ...
    try {
        ...
        $this->db->where('txnid', $txnid)->update('orders', [
            'status' => 'success'
            // 'payment_id' => $payment_id,
            // 'updated_at' => date('Y-m-d H:i:s')
        ]);
