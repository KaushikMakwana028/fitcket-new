...
$(document).ready(function () {
  if ($("#categoryTable").length > 0) {
    ...
    function renderPagination(curr, total) {
      ...
      // Inject into your <ul class="pagination"> outside table
...
$(document).ready(function () {
    ...
    if ($("#PartnerTableContainer").length > 0) {
        ...
        $(document).on("click", "#PartnerTableContainer .btn-toggle-status", function () {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status === "success") {
                                ...
                                loadPartners(offset, $("#PartnerTableContainer .partner-search").val());
                            } else {
        ...
        $(document).on("click", "#PartnerTableContainer .partner-page-link", function (e) {
            e.preventDefault();
        ...
        // ✅ FIXED: Prev button
        $(document).on("click", "#PartnerTableContainer .partner-prev", function (e) {
            e.preventDefault();
            if (offset > 0) {
                offset -= limit;
                loadPartners(offset, $("#PartnerTableContainer .partner-search").val());
        ...
        // ✅ FIXED: Next button
        $(document).on("click", "#PartnerTableContainer .partner-next", function (e) {
            e.preventDefault();
          ...
          if (totalPages > 1) {
            pagination += `<li class="page-item ${
              offset == 0 ? "disabled" : ""
                            ...
                            <a class="page-link" href="javascript:void(0)" data-offset="${
                              offset - limit
                            }">Previous</a></li>`;
            ...
            pagination += `<li class="page-item ${
              offset + limit >= res.total ? "disabled" : ""
            }">
                            <a class="page-link" href="javascript:void(0)" data-offset="${
                              offset + limit
                            }">Next</a></li>`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadTransactions(page = 1) {
    ...
    <div class="d-flex align-items-center 
        ...
        <span>${row.settled == 1 ? "Success" : "Pending"}</span>
                   ...
                   <td>
    <button type="button" 
        onclick="window.location='${site_url}admin/settlement/transaction_details/${
            row.id
          }'" 
  ...
  function renderPagination(current, total) {
    ...
    prev.onclick = () => {
      if (current > 1) loadTransactions(current - 1);
    };
    ...
    next.onclick = () => {
      if (current < total) loadTransactions(current + 1);
    };
                ...
                success: function (res) {
                    if (res.status === "success") {
                        ...
                        if (res.total_pages > 1) {
                            ...
                            pagination += `
                                <li class="page-item ${page == res.total_pages ? "disabled" : ""}">
                        ...
                        $("#SettlementHistoryBody").html(
                            `<tr><td colspan="7" class="text-center">${res.message || "Error loading data"}</td></tr>`
                        );
                    }
                },
                error: function (xhr, status, error) {
                    $("#SettlementHistoryBody").html(
                        `<tr><td colspan="7" class="text-center text-danger">Error: ${error}</td></tr>`
                    );
                },
            });
        }
